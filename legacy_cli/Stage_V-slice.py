...
def navigate_mods(base_path):
    """Navegación por mods con selección de ruta (images o shared/images)"""
    mods_path = base_path  # <-- USAR DIRECTAMENTE base_path
    
    if not os.path.exists(mods_path):
        print(f"{RED}❌ Carpeta de mods no encontrada{RESET}")
        return
    ...
    if isinstance(selected, list) and len(selected) == 1:
        mod_name = selected[0]
        mod_base = os.path.join(mods_path, mod_name)
        
        # ═══════════════════════════════════════════════════
        # PREGUNTAR QUÉ RUTA USAR
        # ═══════════════════════════════════════════════════
        print(f"\n{CYAN}╔══════════════════════════════════════════════════════╗{RESET}")
        print(f"{CYAN}║{MAGENTA}        ¿QUÉ RUTA QUIERES USAR?                   {CYAN}║{RESET}")
        print(f"{CYAN}╚══════════════════════════════════════════════════════╝{RESET}")
        print(f"\n{GREEN}1.{RESET} {mod_name}/images/")
        print(f"{GREEN}2.{RESET} {mod_name}/shared/images/")
        ...
        while True:
            ...
            if opcion == "0":
                ...
                images_path = os.path.join(mod_base, "images")
                ruta_display = f"{mod_name}/images"
                break
            elif opcion == "2":
                images_path = os.path.join(mod_base, "shared", "images")
                ruta_display = f"{mod_name}/shared/images"
                break
            else:
                print(f"{RED}❌ Opción no válida. Elige 0, 1 o 2{RESET}")
        
        # Verificar que exista la ruta elegida
        if not os.path.exists(images_path):
            print(f"\n{RED}❌ La ruta no existe:{RESET}")
            print(f"{CYAN}   {images_path}{RESET}")
            
            # Sugerir la otra ruta si existe
            otra_ruta = None
            if opcion == "1":
                otra_ruta = os.path.join(mod_base, "shared", "images")
                otra_display = f"{mod_name}/shared/images"
            else:
                otra_ruta = os.path.join(mod_base, "images")
                otra_display = f"{mod_name}/images"
            
            if os.path.exists(otra_ruta):
                print(f"\n{YELLOW}💡 ¿Usar la otra ruta disponible?{RESET}")
                print(f"{GREEN}   • {otra_display}/  {CYAN}(existe){RESET}")
                usar_otra = input(f"\n{CYAN}¿Usarla? (s/n): {RESET}").lower()
                if usar_otra == 's':
                    images_path = otra_ruta
                    ruta_display = otra_display
                else:
                    return
            else:
                return
        
        # Entrar al directorio elegido
        print(f"\n{BLUE}🌌 Entrando en MOD: {mod_name}{RESET}")
        print(f"{CYAN}📂 Ruta: {ruta_display}/{RESET}\n")
...
def main():
    ...
    print(f"\n{GREEN}✨=== SPRITE SCALA EDITOR PRO (V-Slice) ===✨{RESET}")
    ...
    print(f"{CYAN}🔧 NUEVO: Selección de ruta al elegir un mod{RESET}")
    
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    base_path = os.path.join(BASE_DIR, "Optimizacion_V-slice/Bgs_stage_scale_vSlice")
    ...
    try:
        ...
        if escalas_globales:
            ...
            if os.path.exists(mods_path):
                ...
                for mod in mods:
                    for sub in [("images",), ("shared", "images")]:
                        images_path = os.path.join(mods_path, mod, *sub)
